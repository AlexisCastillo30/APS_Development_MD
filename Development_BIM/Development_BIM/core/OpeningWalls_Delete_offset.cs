...
namespace Development_BIM.core
{
    ...
    public class OpeningWalls_Delete_offset : IExternalCommand
    {
        private const double OFFSET = 30.0 / 304.8; // 3 cm en pies (unidades internas de Revit)
        private const double RECTANGULAR_INSERTION_OFFSET = 0.0; // Ajustar según sea necesario
        private const double ROUND_INSERTION_OFFSET = 0.0; // Ajustar según sea necesario
                ...
                foreach (Duct duct in ductCollector)
                {
                    ...
                    foreach (Wall wall in potentialWalls)
                    {
                        ...
                        if (intersection != null && intersection.Volume > 0)
                        {
                            ...
                            if (isRoundDuct)
                            {
                                double totalHeight = diameter + 2 * OFFSET;
                                offset = new XYZ(0, 0, -(totalHeight / 2) + ROUND_INSERTION_OFFSET);
                            }
                            else
                            {
                                double totalHeight = height + 2 * OFFSET;
                                offset = new XYZ(0, 0, -(totalHeight / 2) + RECTANGULAR_INSERTION_OFFSET);
                            }
